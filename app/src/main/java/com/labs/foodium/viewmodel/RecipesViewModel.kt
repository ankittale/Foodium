...
class RecipesViewModel @Inject constructor(
    ...
    fun saveMealAndDietType() = viewModelScope.launch(Dispatchers.IO) {
        if (this@RecipesViewModel::mealAndDiet.isInitialized) {
            dataStoreRepository.saveMealAndDietType(
                mealType = mealAndDiet.selectedMealType,
                mealTypeId = mealAndDiet.selectedMealTypeId,
                dietType = mealAndDiet.selectedDietType,
                dietTypeId = mealAndDiet.selectedDietTypeId
            )
        }
    }
    ...
    fun  applyQueries(): HashMap<String, String>{
        ...
        if (this@RecipesViewModel::mealAndDiet.isInitialized) {
            Log.d("RecipesViewModel", "The Queries here if block")
            queries[QUERY_TYPE] = mealAndDiet.selectedMealType
            queries[QUERY_DIET] = mealAndDiet.selectedDietType
        } else {
            Log.d("RecipesViewModel", "The Queries here else block")
            queries[QUERY_TYPE] = DEFAULT_MEAL_TYPE
            queries[QUERY_DIET] = DEFAULT_DIET_TYPE
        }

        Log.d("RecipesViewModel", "The Queries $queries")
        return queries
    }
